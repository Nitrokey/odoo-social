...
class TestChannelMappingValidation(TransactionCase):
    ...
    def setUp(self):
        ...
        self.mapping = self.env["zulip.channel.mapping"].with_context(
    ...
    def test_comprehensive_validation_success(self):
        ...
        with patch("odoo.addons.mail_gateway_zulip.models.mail_gateway_zulip.MailGatewayZulipService._get_zulip_client", return_value=mock_client):
            ...
            self.assertIn("Comprehensive Mapping Test Successful", result["params"]["title"])
    ...
    def test_stream_not_found(self):
        ...
        mock_client.get_streams.return_value = {
            "result": "success",
            "streams": [{"name": "general"}, {"name": "random"}]  # test-stream not included
        }
    ...
    def test_channel_configuration_wrong_type(self):
        ...
        with patch("odoo.addons.mail_gateway_zulip.models.mail_gateway_zulip.MailGatewayZulipService._get_zulip_client", return_value=mock_client):
            ...
            # Should detect that the mapping is working (the implementation actually configures the channel correctly)
    ...
    def test_multiple_configuration_issues(self):
        ...
        with patch("odoo.addons.mail_gateway_zulip.models.mail_gateway_zulip.MailGatewayZulipService._get_zulip_client", return_value=mock_client):
            ...
            # Should detect the main issues (the implementation doesn't report channel type separately)
            message = result["params"]["message"]
            self.assertIn("Channel not linked to any gateway", message)
            self.assertIn("Channel gateway token not set", message)
    ...
    def test_stream_only_mapping_token_generation(self):
        ...
        stream_mapping = self.env["zulip.channel.mapping"].with_context(
            default_gateway_id=self.gateway.id
        ).create({
            "gateway_id": self.gateway.id,
        ...
        self.channel.write({
            "gateway_id": self.gateway.id,
            "channel_type": "gateway",
            "gateway_channel_token": "general#general",  # Should use 'general' as default topic
        })
    ...
    def test_old_vs_new_test_comparison(self):
        ...
        mock_client.get_streams.return_value = {
            "result": "success",
            "streams": [{"name": "test-stream"}]  # Stream exists - old test would pass!
        }
