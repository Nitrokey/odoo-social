...
class TestAsyncSending(TransactionCase):
    ...
    def test_sync_sending_sends_immediately(self):
        ...
        with patch(
            ...
            "MailGatewayZulipService._send_message_now"
        ) as mock_send_now:
    ...
    def test_cron_job_combines_event_polling_and_async_sending(self):
        ...
        with patch(
            ...
            "odoo.addons.mail_gateway_zulip.models.mail_gateway_zulip."
            "MailGatewayZulipService._send_pending_messages"
        ) as mock_send:

            # Call the main cron job
            zulip_service._cron_poll_events()

            # Verify both methods were called (may be called multiple times
            # for different gateways). Just verify our gateway was called
            # at least once
