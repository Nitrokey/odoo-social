...
class ZulipChannelMapping(models.Model):
    ...
    # Odoo Destination
    odoo_channel_id = fields.Many2one("mail.channel", required=True)
    ...
    def _get_zulip_streams_selection(self):
        ...
        try:
            ...
            if streams_response.get("result") == "success":
                ...
                _logger.debug(
                    "Found %d streams for gateway %s", len(stream_list), gateway.name
                )
                return stream_list
            else:
                _logger.warning(
            ...
            _logger.error("Error fetching Zulip streams for selection: %s", str(e))
            import traceback
